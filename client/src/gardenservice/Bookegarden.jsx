...
function Bookegarden() {
    ...
    const [errors, seterrors] = useState({name: '' , email: '' , phone: '' , address: '' , adharcard: ''});
    ...
    const validateform = () => {
        ...
        if(!adharcard){
            newerror.adharcard = 'Goverment ID is required'
        }
        return newerror;
    }
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center">
      <form className="w-full max-w-2xl" onSubmit={handleform}>
        <h2 className="text-center mb-6 sm:mb-9 mt-5 text-2xl sm:text-4xl font-bold overflow-hidden">Book Your Gardening Job</h2>
        <div className="flex flex-col gap-4 sm:gap-5 mb-6 sm:mb-7">
          ...
          <div className="flex flex-col w-full">
            <input
              ...
              value={address}
              placeholder="Address"
              className={`text-black ${errors.address ? 'border-red-500' : 'border-gray-300'} w-full p-2 border bg-white rounded-lg`}
          ...
          <div className="flex flex-col w-full">
            <input
              ...
              className={`text-black ${errors.adharcard ? 'border-red-500' : 'border-gray-300'} w-full p-2 border bg-white rounded-lg`}
              onChange={(e) => setadharcard(e.target.value)}
            />
            {errors.adharcard && <p className='text-red-500 text-sm mt-1'>{errors.adharcard}</p>}
          </div>
        </div>
